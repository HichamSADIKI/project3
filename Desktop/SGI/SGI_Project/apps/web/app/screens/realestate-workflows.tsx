          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            ...
            <tbody>
              {!loading && items.length === 0 && !error && (
                <tr><td colSpan={5} style={{ padding: "24px 16px", textAlign: "center", color: "var(--ink-4)" }}>Aucune instance de workflow.</td></tr>
              )}
              {items.map(w => {
                      ...
                      {w.status === "in_progress" && step ? (
                        busy === w.id ? <span style={{ color: "var(--ink-4)" }}>…</span> : (
                          <span style={{ display: "inline-flex", gap: 6, justifyContent: "flex-end" }}>
                            <button title="Approuver" onClick={() => run(w.id, `/api/admin/workflows/instances/${w.id}/steps/${step.id}/approve`)} style={aBtn("var(--emerald)", "rgba(16,185,129,0.12)")}><IcCheck /></button>
                            <button title="Rejeter" onClick={() => run(w.id, `/api/admin/workflows/instances/${w.id}/steps/${step.id}/reject`)} style={aBtn("var(--rose)", "var(--rose-soft)")}><IcClose /></button>
                          </span>
                        )
