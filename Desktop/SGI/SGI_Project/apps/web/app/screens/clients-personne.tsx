...
function PersonDetail({ person, onBack, lang, onDealConfirmed }: { person: Person; onBack: () => void; lang: string; onDealConfirmed?: (deal: ConfirmedDeal) => void }) {
  ...
  const [newDocType, setNewDocType]   = useState("ID");
  const [newDocStatus, setNewDocStatus] = useState("valid");
  const [newDocDate, setNewDocDate]   = useState(() => new Date().toISOString().slice(0, 10));
  const [newDocSize, setNewDocSize]   = useState("0.5 MB");

  function addDoc() {
    if (!newDocName.trim()) return;
    setDocs(prev => [{ name: newDocName.trim(), type: newDocType, status: newDocStatus, date: newDocDate, size: newDocSize }, ...prev]);
    setShowAddDoc(false);
    setNewDocName(""); setNewDocType("ID"); setNewDocStatus("valid");
    setNewDocDate(new Date().toISOString().slice(0, 10)); setNewDocSize("0.5 MB");
  }
  function deleteDoc(name: string) { setDocs(prev => prev.filter(d => d.name !== name)); }
            ...
            <>
              <div style={{ display: "flex", justifyContent: "flex-end", padding: "10px 16px", borderBottom: "1px solid var(--line-soft)" }}>
                <button onClick={() => setShowAddDoc(true)} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: "var(--r)", background: "var(--gold)", border: "none", color: "#1A1610", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                  <Ic s={13}><path d="M12 5v14M5 12h14"/></Ic>
                  {lang === "ar" ? "إضافة وثيقة" : lang === "fr" ? "Ajouter un document" : "Add Document"}
                </button>
              </div>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead><tr style={{ background: "var(--bg-cream)" }}>
                  {[lang==="ar"?"الوثيقة":lang==="fr"?"Document":"Document", lang==="ar"?"النوع":"Type", lang==="ar"?"الحالة":lang==="fr"?"Statut":"Status", lang==="ar"?"التاريخ":"Date", "Size", ""].map((h, i) => <th key={i} style={thStyle}>{h}</th>)}
                </tr></thead>
                <tbody>{docs.map((d, i) => (
                  <tr key={d.name} style={{ borderBottom: i < docs.length-1 ? "1px solid var(--line-soft)" : "none" }}>
                    ...
                    <td style={tdStyle}><span style={{ fontSize: 10.5, fontWeight: 600, color: DOC_STATUS[d.status]?.color ?? "var(--ink-4)" }}>{DOC_STATUS[d.status]?.en ?? d.status}</span></td>
                    <td style={tdStyle}>{d.date}</td>
        ...
        <div onClick={() => setPreviewDoc(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 1200, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "var(--bg-paper)", borderRadius: "var(--r)", width: 560, maxHeight: "92vh", display: "flex", flexDirection: "column", overflow: "hidden", boxShadow: "0 24px 64px rgba(0,0,0,0.4)" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "16px 20px", borderBottom: "1px solid var(--line-soft)" }}>
              <span style={{ color: "var(--gold)", flexShrink: 0 }}><IcDoc2 /></span>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 700, color: "var(--ink)", fontSize: 14, marginBottom: 5 }}>{previewDoc.name}</div>
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 10.5, padding: "2px 7px", borderRadius: 999, background: "var(--bg-cream)", color: "var(--ink-4)", border: "1px solid var(--line-soft)" }}>{previewDoc.type}</span>
                  <span style={{ fontSize: 10.5, fontWeight: 600, color: DOC_STATUS[previewDoc.status]?.color ?? "var(--ink-4)" }}>{DOC_STATUS[previewDoc.status]?.en ?? previewDoc.status}</span>
                  <span style={{ fontSize: 10.5, color: "var(--ink-4)" }}>{previewDoc.date}</span>
                  <span style={{ fontSize: 10.5, color: "var(--ink-4)" }}>{previewDoc.size}</span>
                </div>
              </div>
              ...
              <div style={{ width: "100%", maxWidth: 430, background: "#fff", borderRadius: 3, boxShadow: "0 3px 16px rgba(0,0,0,0.18)", padding: "36px 44px 40px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", paddingBottom: 14, marginBottom: 20, borderBottom: "2px solid #C8A03C" }}>
                  ...
                  <div style={{ textAlign: "end" }}>
                    <div style={{ fontSize: 10, color: "#8B7355" }}>{previewDoc.date}</div>
                    <div style={{ fontSize: 10, fontWeight: 700, color: "#C8A03C", marginTop: 2, padding: "1px 6px", border: "1px solid #C8A03C", borderRadius: 4 }}>{previewDoc.type}</div>
                  </div>
                </div>
                <div style={{ fontWeight: 700, fontSize: 15, color: "#1A1610", marginBottom: 18 }}>{previewDoc.name}</div>
                <div style={{ fontSize: 10, color: "#8B7355", marginBottom: 14 }}>{lang === "ar" ? "العميل" : lang === "fr" ? "Client" : "Client"}: {person.name}</div>
                ...
                <div style={{ marginTop: 28, display: "flex", justifyContent: "space-between" }}>
                  <div style={{ borderTop: "1px solid #ccc", paddingTop: 8, fontSize: 9, color: "#8B7355", width: 110, textAlign: "center" }}>
                    {lang === "ar" ? "توقيع العميل" : lang === "fr" ? "Signature client" : "Client signature"}
                  </div>
                  <div style={{ borderTop: "1px solid #ccc", paddingTop: 8, fontSize: 9, color: "#8B7355", width: 110, textAlign: "center" }}>
                    {lang === "ar" ? "توقيع الوكيل" : lang === "fr" ? "Signature agent" : "Agent signature"}
                  </div>
                </div>
              </div>
              {/* Page 2 (partial) */}
        ...
        <div onClick={() => setShowAddDoc(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 1200, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "var(--bg-paper)", borderRadius: "var(--r)", width: 420, boxShadow: "0 24px 64px rgba(0,0,0,0.4)", overflow: "hidden" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", borderBottom: "1px solid var(--line-soft)" }}>
              <span style={{ fontWeight: 700, fontSize: 14, color: "var(--ink)" }}>{lang === "ar" ? "إضافة وثيقة" : lang === "fr" ? "Ajouter un document" : "Add Document"}</span>
            ...
            <div style={{ padding: "20px", display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {/* Name */}
              <div>
                <label style={{ fontSize: 11, color: "var(--ink-4)", display: "block", marginBottom: 5 }}>{lang === "ar" ? "الاسم" : lang === "fr" ? "Nom" : "Name"} *</label>
                <input value={newDocName} onChange={e => setNewDocName(e.target.value)} placeholder={lang === "ar" ? "اسم الوثيقة" : lang === "fr" ? "Nom du document" : "Document name"} style={{ width: "100%", padding: "8px 10px", borderRadius: "var(--r)", border: "1px solid var(--line-soft)", background: "var(--bg-cream)", color: "var(--ink)", fontSize: 13, boxSizing: "border-box" }} />
              </div>
              {/* Type + Status */}
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <label style={{ fontSize: 11, color: "var(--ink-4)", display: "block", marginBottom: 5 }}>Type</label>
                  <select value={newDocType} onChange={e => setNewDocType(e.target.value)} style={{ width: "100%", padding: "8px 10px", borderRadius: "var(--r)", border: "1px solid var(--line-soft)", background: "var(--bg-cream)", color: "var(--ink)", fontSize: 13 }}>
                    {["ID", "Property", "Contract", "Visa", "Finance", "Other"].map(t => <option key={t}>{t}</option>)}
                  </select>
                </div>
                <div>
                  <label style={{ fontSize: 11, color: "var(--ink-4)", display: "block", marginBottom: 5 }}>{lang === "ar" ? "الحالة" : lang === "fr" ? "Statut" : "Status"}</label>
                  <select value={newDocStatus} onChange={e => setNewDocStatus(e.target.value)} style={{ width: "100%", padding: "8px 10px", borderRadius: "var(--r)", border: "1px solid var(--line-soft)", background: "var(--bg-cream)", color: "var(--ink)", fontSize: 13 }}>
                    {["valid", "pending", "signed", "submitted", "draft"].map(s => <option key={s} value={s}>{DOC_STATUS[s]?.en ?? s}</option>)}
                  </select>
                </div>
              </div>
              {/* Date */}
              <div>
                <label style={{ fontSize: 11, color: "var(--ink-4)", display: "block", marginBottom: 5 }}>{lang === "ar" ? "التاريخ" : "Date"}</label>
