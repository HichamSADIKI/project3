...
/**
 * Import CSV de biens — bouton + sélecteur + rapport. Auto-suffisant : POST
 * multipart vers /api/admin/properties/import (les colonnes latitude/longitude
 * géolocalisent le bien côté PostGIS). CSS logique (Loi 3). i18n local.
 */
...
export function PropertyCsvImport({ onDone }: { onDone?: () => void }): React.ReactNode {
  const { lang } = useLang();
  ...
  async function onFile(e: React.ChangeEvent<HTMLInputElement>): Promise<void> {
    ...
    try {
      ...
      fd.append("file", file);
      const res = await postForm("/api/admin/properties/import", fd);
      if (!res.ok) {
