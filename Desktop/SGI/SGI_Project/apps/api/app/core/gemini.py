...
_CATEGORY_KEYWORDS: dict[str, tuple[str, ...]] = {
    "realestate": (
        ...
        "townhouse", "logement", "maison", "terrain", "bureau", "local commercial",
        "achat immobilier", "acheter un bien", "vendre mon bien", "à louer",
        "real estate", "immobilier", "bien immobilier", "propriété",
        "loyer", "hypothèque immobilière", "dld", "ejari", "freehold",
        "marina", "downtown", "palm", "jbr", "deira", "jvc", "jlt", "fila",
        "شقة", "فيلا", "إيجار", "شراء", "عقار", "منزل",
    ),
    "tourisme": (
        "hotel", "hôtel", "tour", "visa touristique", "yacht", "excursion",
        "circuit", "séjour", "tourisme", "tourism", "voyage", "voyager",
        "transfer", "transfert aéroport", "vip",
        # voyage aérien
        "billet", "billet d'avion", "billet davion", "avion", "vols",
        "aérien", "aerien", "compagnie aérienne", "flight", "plane ticket",
        "airline", "airfare", "ticket avion",
        # loisirs / séjour
        "croisière", "cruise", "plage", "resort", "vacances", "holiday",
        "réservation hôtel", "safari", "desert safari", "billetterie",
        "فندق", "سياحة", "رحلة", "تأشيرة", "طيران", "تذكرة طيران", "رحلة جوية",
    ),
    "sante": (
        "medical", "doctor", "médecin", "docteur", "dentiste", "dentist",
        "hospital", "hôpital", "clinique", "clinic", "surgery", "chirurgie",
        "health insurance", "santé", "médical", "checkup", "bilan", "pharmacie",
        "traitement", "consultation médicale", "ophtalmo", "fertilité", "ivf",
        "طبي", "صحة", "مستشفى", "طبيب", "عيادة",
    ),
    "assurance": (
        "insurance", "assurance", "assurer", "policy", "police d'assurance",
        "couverture", "claim", "sinistre", "mutuelle", "assurance auto",
        "assurance habitation", "assurance vie", "assurance voyage",
        "تأمين", "بوليصة",
    ),
    "banques": (
        "mortgage", "loan", "account", "compte bancaire", "bank", "wealth",
        "credit", "banque", "compte", "prêt", "crédit", "hypothèque",
        "carte bancaire", "virement", "iban", "financement", "épargne",
        "investissement", "gestion de patrimoine", "bourse",
        "بنك", "قرض", "حساب", "رهن", "تمويل",
    ),
    "amazon": (
        "amazon", "fba", "e-commerce", "ecommerce", "store", "shopify",
        "boutique en ligne", "vendeur en ligne", "marketplace", "dropshipping",
        "vendre en ligne", "noon", "fulfillment",
        "أمازون", "متجر إلكتروني", "تجارة إلكترونية",
    ),
    "consultants": (
        "consulting", "consultant", "audit", "strategy", "due diligence",
        "conseil", "stratégie", "accompagnement", "expertise", "business plan",
        "étude de marché", "coaching", "feasibility",
        "استشارة", "تدقيق", "خطة عمل",
    ),
    "admin": (
        "license", "licence", "permit", "company setup", "trade license",
        "pro service", "notary", "création société", "creation societe",
        "créer une société", "permis", "notaire", "résidence", "golden visa",
        "emirates id", "immigration", "attestation", "pro services", "freezone",
        "ترخيص", "تأسيس شركة", "إقامة", "هوية إماراتية",
    ),
    "travail": (
        "recruitment", "hiring", "headhunting", "employment", "rh",
        "recrutement", "recruter", "recrute", "emploi", "embauche", "embaucher",
        "ressources humaines", "candidat", "cv", "offre d'emploi", "staffing",
        "main d'oeuvre", "manpower",
        "توظيف", "وظيفة", "موارد بشرية",
    ),
}
...
_PROPERTY_TYPES: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("commercial", (
        "bureau", "local commercial", "commercial", "office", "shop",
        "magasin", "مكتب", "محل",
    )),
...
_KNOWN_LOCATIONS: tuple[str, ...] = (
    ...
    "dubai hills", "emirates hills", "arabian ranches", "dubai silicon oasis",
    "al barsha", "yas island", "saadiyat island", "al reem island",
    "abu dhabi", "jbr", "jvc", "jlt", "deira", "bur dubai", "mirdif",
    "marina", "downtown", "jumeirah", "palm", "dubai", "sharjah", "ajman",
    "ras al khaimah", "fujairah", "umm al quwain",
    "نخلة جميرا", "دبي", "أبوظبي", "الشارقة",
)

_URGENCY_HIGH: tuple[str, ...] = (
    "urgent", "asap", "au plus vite", "immédiat", "immediat", "tout de suite",
    "rapidement", "dès que possible", "des que possible", "ce mois",
    "cette semaine", "this month", "this week", "immediately", "right away",
    "عاجل", "بسرعة", "فورا", "في أقرب وقت",
)
_URGENCY_LOW: tuple[str, ...] = (
    "pas pressé", "pas presse", "pas urgent", "flexible", "plus tard",
    "no rush", "long terme", "long term", "dans quelques mois",
    "éventuellement", "eventuellement", "غير مستعجل", "لاحقا",
)
...
async def extract_trade_licence(
    ...
    if not document_bytes or not any(
        mime.startswith(p) for p in _LICENCE_MIME_PREFIXES
    ):
