...
async def _company(db_session, suffix: str) -> uuid.UUID:
    cid = uuid.uuid4()
    db_session.add(
        Company(id=cid, name=f"Soc {suffix}", slug=f"{suffix}-{cid.hex[:8]}",
...
async def test_two_companies_can_share_reference(db_session):
    ...
    db_session.add_all(
        [
            Property(id=uuid.uuid4(), company_id=a, reference=ref,
                     type="apartment", price=1, status="available"),
            Property(id=uuid.uuid4(), company_id=b, reference=ref,
                     type="villa", price=2, status="available"),
        ]
    )
...
async def test_same_company_cannot_reuse_reference(db_session):
    ...
    db_session.add(
        Property(id=uuid.uuid4(), company_id=a, reference=ref,
                 type="plot", price=3, status="available")
    )
