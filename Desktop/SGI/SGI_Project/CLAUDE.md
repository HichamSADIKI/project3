...
Alembic migrations live in **`apps/api/migrations/versions/`** (not the default `alembic/versions/`). Numbered `NNNN_name.py`; head is `0036_iam`. Worker/migrations use the privileged `sgi_user`; the API uses restricted `sgi_app` (see Law 1).
...
**Frontend** : la rubrique **Immobilier** (`apps/web`, `components/sgi-ui.tsx` + `app/screens/realestate-*.tsx`) regroupe ses entrées de nav en **5 sections thématiques** (clé `section` sur chaque `NavItem`, libellés `nav_re_sec_*` AR/EN/FR dans `lib/i18n.ts`) : `patrimoine` (Patrimoine — Bâtiments · Unités · Succursales), `transactions` (Transactions & contrats — CRM · Achat · Vente · Location · Contrats…), `tiers_finance` (Tiers & finance), `exploitation` (Exploitation & relation client) et `admin`. Migration de merge `0026_merge_0025` réconcilie le fork `owner_statements` / `reference_composite_unique`.
...
- Alembic migrations live in [apps/api/migrations/versions/](apps/api/migrations/versions/) (NOT `alembic/versions/`) — 0001 → 0036. `make migrate` runs `alembic upgrade head` via the privileged `sgi_user` role.
