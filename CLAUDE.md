...
Alembic migrations live in **`apps/api/migrations/versions/`** (not the default `alembic/versions/`). Numbered `NNNN_name.py`; head is `0061_public_site_design` (64 migrations, 0001 → 0061). Worker/migrations use the privileged `sgi_user`; the API uses restricted `sgi_app` (see Law 1).
...
- Alembic migrations live in [apps/api/migrations/versions/](apps/api/migrations/versions/) (NOT `alembic/versions/`) — 0001 → 0061. `make migrate` runs `alembic upgrade head` via the privileged `sgi_user` role.
